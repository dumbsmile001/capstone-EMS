        ...
        <div class="relative bg-gradient-to-br from-blue-50 to-yellow-50 shadow-xs rounded-xl border border-blue-100">
                        ...
                        <div class="md:col-span-2">
                            <div class="relative group">
                                <div class="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
                                    <svg class="w-4 h-4 text-blue-400 group-focus-within:text-yellow-500 transition-colors"
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-blue-100">
                    <thead class="bg-gradient-to-r from-blue-500 to-blue-900">
                        <tr>
                    ...
                    <tbody class="bg-white divide-y divide-blue-50">
                        @forelse($users as $index => $user)
                            <tr
                                class="{{ $index % 2 === 0 ? 'bg-white hover:bg-blue-50' : 'bg-blue-50/30 hover:bg-blue-100' }} transition-colors duration-150 group">
                                <td class="px-4 py-3 text-sm font-medium text-gray-900">{{ $user->first_name }}
                                    {{ $user->last_name }}</td>
                                <td class="px-4 py-3 text-sm text-gray-600">{{ $user->email }}</td>
                                <td class="px-4 py-3 text-sm text-gray-600">
                                    @if ($user->grade_level)
                                        <span
                                            class="px-2 py-1 bg-blue-100 text-blue-800 rounded-lg text-xs font-medium">{{ $user->grade_level }}</span>
                                    @else
                                ...
                                <td class="px-4 py-3 text-sm text-gray-600">
                                    @if ($user->year_level)
                                        <span
                                            class="px-2 py-1 bg-blue-100 text-blue-800 rounded-lg text-xs font-medium">{{ $user->year_level }}</span>
                                    @else
                                        <span class="text-gray-400">—</span>
                                    @endif
                                </td>
                                <td class="px-4 py-3 text-sm text-gray-600">
                                    @if ($user->shs_strand)
                                        <span
                                            class="px-2 py-1 bg-yellow-100 text-yellow-800 rounded-lg text-xs font-medium">{{ $user->shs_strand }}</span>
                                    @else
                                        <span class="text-gray-400">—</span>
                                    @endif
                                </td>
                                <td class="px-4 py-3 text-sm text-gray-600">
                                    @if ($user->college_program)
                                        <span
                                            class="px-2 py-1 bg-yellow-100 text-yellow-800 rounded-lg text-xs font-medium">{{ $user->college_program }}</span>
                                    @else
                                        <span class="text-gray-400">—</span>
                                    @endif
                                </td>
                                ...
                                <td class="px-4 py-3 text-sm">
                                    @foreach ($user->roles as $role)
                                        <span
                                            class="px-2 py-1 {{ $role->name === 'admin' ? 'bg-purple-100 text-purple-800' : ($role->name === 'organizer' ? 'bg-yellow-100 text-yellow-800' : 'bg-blue-100 text-blue-800') }} rounded-lg text-xs font-medium capitalize">
                                            {{ $role->name }}
                                        </span>
                                    @endforeach
                                </td>
                            ...
                            <tr>
                                <td colspan="9" class="px-4 py-12 text-center text-gray-500">
                                    <div class="flex flex-col items-center justify-center">
                                        <svg class="w-16 h-16 mb-4 text-blue-300" fill="none"
                        ...
                        <div class="flex items-center space-x-2">
                            ...
                            @else
                                <button wire:click="previousPage"
                                    class="px-3 py-2 bg-white text-blue-600 border-2 border-blue-200 rounded-lg hover:bg-yellow-400 hover:text-white hover:border-yellow-400 transition-all duration-200">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M15 19l-7-7 7-7" />
                                    </svg>
