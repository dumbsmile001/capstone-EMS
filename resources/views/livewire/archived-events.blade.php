{{-- resources/views/livewire/archived-events.blade.php --}}
<div class="flex min-h-screen bg-gray-50">
    ...
    <div class="flex-1 lg:ml-64 overflow-y-auto overflow-x-hidden">
        ...
        <div class="flex-1 p-6 mt-20 lg:mt-24">
            ...
            <div class="mb-6">
                <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    <div>
                        <div class="flex items-center gap-3 mb-2">
                            ...
                            <h1 class="text-3xl font-bold text-gray-800">Archived Events</h1>
                        </div>
                        <p class="text-gray-600">View and manage your archived events</p>
                    </div>
                ...
                <!-- Stats Cards (Organizer-specific stats) -->
            ...
            <!-- Enhanced Search and Filter Section (identical to admin) -->
            <div class="mb-6 bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                ...
                <div class="p-4 bg-gradient-to-r from-blue-600 to-yellow-500 border-b border-gray-100">
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-2">
                            ...
                            <h2 class="text-lg font-semibold text-white">Filter Archived Events</h2>
                        </div>
                ...
                <div class="p-5">
                    ...
                    <!-- Filter Grid (Organizer doesn't need Creator filter) -->
                    ...
                    <!-- Bottom Filter Row: Sort and Per Page (like admin) -->
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @foreach ($events as $index => $event)
                                    <tr
                                        class="{{ $index % 2 === 0 ? 'bg-white' : 'bg-blue-50/30' }} hover:bg-yellow-50/50 transition-colors">
                                        <td class="px-6 py-4">
                                            <div class="flex items-center">
                                                ...
                                                <div>
                                                    ...
                                                    <div class="text-xs text-gray-500">
                                                        {{ Str::limit($event->description, 40) }}</div>
                                                </div>
                                            </div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="text-sm font-medium text-gray-900">
                                                {{ $event->start_date->format('M d, Y') }}</div>
                                            <div class="text-xs text-gray-500">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="flex flex-col gap-1.5">
                                                <span
                                                    class="px-2.5 py-1 text-xs rounded-full bg-blue-100 text-blue-700 font-medium w-fit">
                                                    {{ ucfirst($event->category) }}
                                                </span>
                                                <span
                                                    class="px-2.5 py-1 text-xs rounded-full bg-yellow-100 text-yellow-700 font-medium w-fit">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="text-sm font-semibold text-gray-900">
                                                {{ $event->registrations_count }}</div>
                                            <div class="text-xs text-gray-500">registrations</div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="text-sm font-medium text-gray-900">
                                                {{ $event->archived_at->format('M d, Y') }}</div>
                                            <div class="text-xs text-gray-500">
                                                {{ $event->archived_at->format('g:i A') }}</div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="flex items-center gap-2">
                                                <!-- Restore Icon Button -->
                                                <button wire:click="confirmRestore({{ $event->id }})"
                                                    ...
                                                    <span
                                                        class="absolute -top-8 left-1/2 transform -translate-x-1/2 bg-gray-800 text-white text-xs py-1 px-2 rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap">
                                                        Restore
                                                    </span>
                                                </button>
                ...
                <div class="text-center py-16 bg-white rounded-xl shadow-sm border border-gray-200">
                    <div class="flex justify-center mb-4">
                        <div class="p-4 bg-blue-100 rounded-full">
                            <svg class="h-16 w-16 text-blue-500" fill="none" stroke="currentColor"
                                viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <p class="mt-1 text-sm text-gray-500 max-w-md mx-auto">
                        @if ($search || $filterCategory || $filterType || $filterPayment)
                            No events match your current filters. Try adjusting your search criteria.
                        @else
                            You don't have any archived events yet. Archived events will appear here when you archive them.
                        @endif
                    </p>
                    @if ($search || $filterCategory || $filterType || $filterPayment)
                        <div class="mt-6">
    ...
    <!-- Export Modal (identical to admin) -->
    <x-custom-modal model="showExportModal" maxWidth="lg" title="Export Archived Events"
        description="Export your archived events data with current filters" headerBg="green">
        <div class="space-y-6">
                ...
                <div class="grid grid-cols-2 gap-3 text-sm">
                    <div class="space-y-2">
                        ...
                        <p class="text-green-700">
                            ...
                            <span
                                class="text-green-600">{{ $filterCategory ? ucfirst($filterCategory) : 'All' }}</span>
                        </p>
                    </div>
                    <div class="space-y-2">
                        <p class="text-green-700">
    ...
    <x-confirmation-modal model="showRestoreConfirmation" title="Restore Event" :message="'Are you sure you want to restore the event &quot;' .
        $selectedEventTitle .
        '&quot;? This will make it active again.'"
        confirmButtonText="Yes, Restore" confirmButtonColor="green" />

    <!-- Delete Confirmation Modal -->
    <x-confirmation-modal model="showDeleteConfirmation" title="Permanently Delete Event" :message="'Are you sure you want to permanently delete &quot;' .
        $selectedEventTitle .
        '&quot;? This action cannot be undone.'"
        confirmButtonText="Yes, Delete Permanently" confirmButtonColor="red" />
