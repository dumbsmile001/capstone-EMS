...
class AdminEventAttendance extends Component
{
    use WithPagination;
    ...
    public function exportAttendance()
    {
        ...
        $data = $attendance->map(function ($ticket) {
            return [
                ...
                'Event Organizer' => $ticket->registration->event->creator->first_name . ' ' . $ticket->registration->event->creator->last_name,
        ...
        $filename = 'admin_attendance_report_' . date('Y-m-d_H-i-s');
        
        // Add filter info to filename if filters are applied
        if ($this->search || $this->filterEvent || $this->filterDate || $this->filterOrganizer) {
            $filename .= '_filtered';
        }
        ...
        if ($this->exportFormat === 'csv') {
            ...
            return response()->streamDownload(function () use ($data) {
                ...
                $spreadsheet->getProperties()
                    ...
                    ->setTitle("Admin Attendance Report")
                    ->setSubject("Event Attendance Data Export")
                    ->setDescription("Exported attendance data from admin dashboard");
    ...
    public function getFilteredAttendanceQuery()
    {
        return Ticket::where('status', 'used')
            ...
            ->when($this->filterOrganizer, function ($query) {
                $query->whereHas('registration.event', function ($eventQuery) {
                    $eventQuery->where('created_by', $this->filterOrganizer);
                });
            })
            ->with([
                'registration.event' => function ($query) {
                    $query->with('creator');
                },
                'registration.user',
    ...
    {
        $this->reset(['search', 'filterEvent', 'filterPayment', 'filterDate', 'filterOrganizer', 'perPage']);
        $this->resetPage();
    }
    ...
    public function render()
    {
        ...
        // Get all events that have attendance records
        $events = \App\Models\Event::whereHas('registrations.ticket', function ($query) {
                $query->where('status', 'used');
            })
            ->orderBy('title')
            ->get();
        
        // Get all organizers who have created events with attendance
        $organizers = \App\Models\User::whereHas('events.registrations.ticket', function ($query) {
                $query->where('status', 'used');
            })
            ->orderBy('first_name')
            ->get();

        return view('livewire.admin-event-attendance', [
